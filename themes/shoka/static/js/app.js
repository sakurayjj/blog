...
const pageScroll = function(target, offset, complete) {
  var scrollTop = offset || (typeof target == 'number' ? target : (target ? target.top() + document.documentElement.scrollTop - siteNavHeight : 0))
  if (!window.anime) {
...
const transition = function(target, type, complete) {
  ...
  if (!window.anime) {
    if (type === 1 || type === 'shrinkIn' || type === 'bounceUpIn' || type === 'slideRightIn' || type === 'slideLeftIn' || type === 'slideUpIn' || type === 'slideDownIn') {
      ...
      target.display('none')
    } else if (type && type.display !== undefined) {
...
const lazyload = window.lozad
  ? window.lozad('img, [data-background-image]', {
      loaded: function(el) {
        el.addClass('lozaded');
      }
    })
